...
def bank_statement_read(file_path, bank_name):
    ...
    try:
        ...
        for info in range(len(Transaction_Date)):
            ...
            deb = Debit[info]
            bal = Balance[info]
            """if re.findall("Dr", cred) or re.findall("Dr", cred) or re.findall("Dr", deb) or re.findall("Dr", deb) or re.findall("DR", cred) or re.findall("DR", cred) or re.findall("DR", deb) or re.findall("DR", deb):
                rmk= "DEBIT"
                cred=0.0
                deb=deb
            elif re.findall("Cr", cred) or re.findall("Cr", cred) or re.findall("Cr", deb) or re.findall("Cr", deb) or re.findall("CR", cred) or re.findall("CR", cred) or re.findall("CR", deb) or re.findall("CR", deb):
                rmk = "CREDIT"
                cred=cred
                deb=0.0
            else:
            ...
            try:
                ...
                amount = 0.0 
                remark = "None"                         
            txn_count= {"transactionNumber":info,"dateTime":txn,"description":narr,"amount":amount,"batchID":batch_id,
            "balanceAfterTransaction":bal,"category":np.nan,"valueDate":np.nan,
            "transactionId":uuid.uuid1(),"bank":bank_name,"remark":np.nan,"type":remark}
