import numpy as np, tabula, pandas as pd, operator as op, sys, json, re
...
def bank_statement_read(file_path, bank_name):
    ...
    try:
        ...
        for info in range(len(Transaction_Date)):
            ...
            deb = Debit[info]
            bal = Balance[info]  
            txn_count= {"Sr. No.":info,"Transaction_Date":txn,"Narration":narr,"Credit":cred,
            "Debit":deb, "Balance":bal,"Transaction Category":np.nan}
            output.append(txn_count)
            for carr in range(len(output)):
                serch=str(output[carr]["Narration"])
                for upi in range(len(UPI)):
                    upi_serch=str(UPI[upi])
                    if re.findall(upi_serch, serch):output[carr]["Transaction Category"] = "UPI"
                for wt in range(len(widrawal)):
                    wt_serch=str(widrawal[wt])
                    if re.findall(wt_serch, serch):output[carr]["Transaction Category"] = "ATM Withdrawal"
                if re.findall("NEFT", serch):output[carr]["Transaction Category"] = "NEFT" 
                if re.findall("IMPS", serch):output[carr]["Transaction Category"] = "IMPS"
                for ecom in range(len(E_Commerse)):
                    ecom_serch=str(E_Commerse[ecom])
                    if re.findall(ecom_serch, serch):output[carr]["Transaction Category"] = "E_Commerse"                                
                for sal in range(len(Salary)):
                    sal_serch=str(Salary[sal])
                    if re.findall(sal_serch, serch):output[carr]["Transaction Category"] = "Salary"
                for nach in range(len(NACH)):
                    nach_serch=str(NACH[nach])
                    if re.findall(nach_serch, serch):output[carr]["Transaction Category"] = "NACH"
                for bnach in range(len(NACH_Bounce)):
                    bnach_serch=str(NACH_Bounce[bnach])
                    if re.findall(bnach_serch, serch):output[carr]["Transaction Category"] = "NACH_Bounce"
                for gross in range(len(Online_Grosery)):
                    gross_serch=str(Online_Grosery[gross])
                    if re.findall(gross_serch, serch):output[carr]["Transaction Category"] = "Online_Grosery"
                for food in range(len(Online_FOOD)):
                    food_serch=str(Online_FOOD[food])
                    if re.findall(food_serch, serch):output[carr]["Transaction Category"] = "Online_FOOD"
                for bet in range(len(Btting_sites)):
                    bet_serch=str(Btting_sites[bet])
                    if re.findall(bet_serch, serch):output[carr]["Transaction Category"] = "Beting_sites"
                for fan in range(len(Gambling_sites)):
                    fan_serch=str(Gambling_sites[fan])
                    if re.findall(fan_serch, serch):output[carr]["Transaction Category"] = "Fantacy_Gamming" 
                if re.findall("RAZOR", serch):output[carr]["Transaction Category"] = "RAZORPAY"
                if re.findall("RZP", serch):output[carr]["Transaction Category"] = "RAZORPAY"
                if re.findall("INB", serch):output[carr]["Transaction Category"] = "INTERNET BANKING TRANSACTION" 
                if re.findall("RENT", serch):output[carr]["Transaction Category"] = "RENT"
                if re.findall("bill", serch):output[carr]["Transaction Category"] = "BILL PAYMENT"   
                if re.findall("BILL", serch):output[carr]["Transaction Category"] = "BILL PAYMENT" 
                if re.findall("POS ", serch):output[carr]["Transaction Category"] = "Card Transaction" 
                if re.findall("debit card", serch):output[carr]["Transaction Category"] = "Card Transaction"
                if re.findall("CHQ DEP", serch):output[carr]["Transaction Category"] = "CHQ DEP" 
